#!/usr/bin/env python3
"""
This module manages product operations within the inventory system. It provides functionality for
adding, updating, producing (which includes both manufacturing and selling), deleting, and querying
products. Each product is stored with unique identifiers and associated details such as name,
quantity, recipe, and sale price. Operations ensure data integrity and consistency by validating
inputs and checking the availability of necessary components for production.

The module interfaces with the `components` module to validate component availability for product
recipes and the `utilities` module for logging and generating unique identifiers.

Functions:
    add_product(data): Adds a new product to the inventory.
    update_product(data): Updates details of an existing product.
    produce_product(data): Adjusts the inventory quantity of a product based on production or sales.
    delete_product(data): Removes a product from the inventory.
    get_product(data): Retrieves detailed information about a specific product.
    get_quantity_of_product(data): Retrieves the current stock quantity of a specified product.
"""
...
def add_product(data):
    ...
    Parameters:
    - data (dict): A dictionary containing the new product's details such as name,
        quantity, recipe, and sale price.

    Returns:
    - dict: A response object with a status message indicating success or error,
        and the new product's ID if successful.
    """
    ...
    production_cost = sum(components[recipe_item.get('ComponentID')]["ProductionCost"]
                      * recipe_item.get('Quantity') for recipe_item in recipe)
    products[product_id] = {"Name": name, "Quantity": quantity, "Recipe": recipe,
                            "ProductionCost": production_cost, "SalePrice": sale_price}
...
def update_product(data):
    ...
    Parameters:
    - data (dict): A dictionary containing the product's ID and
        new details (name, quantity, recipe, and sale price).

    Returns:
    - dict: A response object with a status message indicating
        success or error, and a message about the update.
    """
    ...
    if (not product_id or not new_name or new_quantity is None or
        not new_recipe or new_sale_price is None):
    ...
    if product_id in products:
        for recipe_item in new_recipe:
            ...
            if recipe_item.get('Quantity') is None:
                ...
                return {"status": "error", "message": "Missing quantity for component"}
        production_cost = sum(components[recipe_item.get('ComponentID')]["ProductionCost"]
                      * recipe_item.get('Quantity') for recipe_item in new_recipe)
        products[product_id] = {"Name": new_name, "Quantity": new_quantity, "Recipe":
                                new_recipe, "ProductionCost":
        production_cost, "SalePrice": new_sale_price}
...
def produce_product(data):
    ...
    Parameters:
    - data (dict): Contains the product's ID and the quantity to adjust by.
        Quantity can be negative for sales.

    Returns:
    - dict: A response object indicating the success or failure of the
        operation, and the updated quantity if successful.
    """
    product_id = data.get('ProductID')
    quantity = data.get('Quantity')
    ...
    for recipe_item in recipe:
        ...
        if components[component_id]["Quantity"] - number_of_components_needed < 0:
            ...
            return {"status": "error", "message":
                    f"Insufficient quantity for component {component_id}"}
    for recipe_item in recipe:
        component_id = recipe_item.get('ComponentID')
...
def get_product(data):
    ...
    Returns:
    - dict: A response object including the product's details if successful,
    ...
    if product_id in products:
        ...
        return {"status": "success", "data":
                {"ProductID": product_id, "Name": products[product_id]["Name"],
                 "SalePrice": products[product_id]["SalePrice"], "ProductionCost":
        products[product_id]["ProductionCost"], "Recipe": products[product_id]["Recipe"]}}
    log(f"Product not found: {product_id}")
    return {"status": "error", "message": "Product not found"}
...
def get_quantity_of_product(data):
    ...
    Returns:
    - dict: A response object including the current quantity of the product if found,
         or an error message if not.
    """
    product_id = data.get('ProductID')
    if product_id in products:
