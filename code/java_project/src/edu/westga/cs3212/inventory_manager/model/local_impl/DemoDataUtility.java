...
public final class DemoDataUtility {
	...
	/**
	 * Generates a map of demo components with randomized quantities. Each component
	 * is created with a unique name and a randomized price. Quantities range from 1
	 * to 100.
	 *
	 * @preconditions none
	 * @postconditions none
	 *
	 * @return a Map<Component, Integer> where each key is a Component object and
	...
	/**
	 * Generates a map of demo products based on the given components. Each product
	 * is created with a unique name, a production cost, a sale price, and a recipe
	 * consisting of 2 to 5 components. Product stock quantities range from 1 to 50.
	 *
	 * @param components a Map<Item, Integer> representing the available components
	 *                   and their quantities to be used in creating products
	 * @preconditions components != null && !components.isEmpty()
	 * @postconditions none
	 *
	 * @return a Map<Product, Integer> where each key is a Product object and each
	 *         value is the stock quantity of that product.
	 */
	static Map<Product, Integer> createDemoProducts(Map<Item, Integer> components) {
		String[] productNames = { "Iron Pickaxe", "Stone Shovel", "Wooden Axe", "Diamond Sword", "Gold Hoe",
				"Steel Hammer", "Copper Wrench", "Aluminum Ladder", "Plastic Pipe", "Glass Bottle", "Rubber Belt",
				"Silicon Chip", "Gold Necklace", "Silver Ring", "Bronze Statue", "Lead Pipe", "Nickel Screw",
				"Zinc Plate", "Tin Can", "Magnesium Fire Starter", "Titanium Frame", "Brass Knuckles", "Quartz Watch",
				"Marble Sculpture", "Ceramic Vase" };

		Map<Product, Integer> products = new HashMap<>();
		for (int i = 0; i < productNames.length; i++) {
			// Prices between 0.00 and 100.00
			double productionCost = Math.round(Math.random() * 100.0 * 100.0) / 100.0;

			// Sale prices between 0.00 and 150.00, assuming higher than production cost
			double salePrice = Math.round(Math.random() * 150.0 * 100.0) / 100.0;
	 ...
	 * filled with 1 to 10 products, and each product quantity ranges from 1 to 10.
	 * Orders are randomly marked as either COMPLETE or INCOMPLETE.
	 *
	 * @param products a Map<Product, Integer> representing the available products
