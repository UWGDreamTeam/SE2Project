...
public class AddProduct {
	    ...
	    void addProductOnClick(ActionEvent event) {
			try {
				this.addProductVM.addProduct(componentList);
			} catch (Exception e) {
		...
		void initialize() {
			...
			this.addProductVM.getSelectedComponentProperty().bind(this.componentRecipeTableView.getSelectionModel().selectedItemProperty());
		...
		private void setupComponentTableView() {
			...
			this.componentRecipeTableView.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
		        if (newSelection != null) {
		            Integer quantity = this.componentList.getOrDefault(newSelection, 0); // Default to 1 if not found
		            // This line assumes your Spinner is configured with a SpinnerValueFactory that allows setting its value
