...
public class OrderPage {
	...
	BooleanBinding isClosedOrdersTabSelected = this.ordersTabPane.getSelectionModel().selectedIndexProperty()
		.isEqualTo(1);
	...
	this.openFulfillmentStatusColumn.setCellValueFactory(cellData -> {
	    String status = cellData.getValue().getCompletionStatus() == CompletionStatus.COMPLETE ? "Completed"
		    : "Incomplete";
	    return new SimpleStringProperty(status);
	});

	this.openSalesPriceColumn.setCellValueFactory(
	...
	this.openProductionCostColumn.setCellValueFactory(
		cellData -> new SimpleStringProperty(String.format("%.2f", cellData.getValue().getProductionCost())));
	...
	this.closedSalesPriceColumn.setCellValueFactory(
		cellData -> new SimpleStringProperty(String.format("%.2f", cellData.getValue().getSalePrice())));
